// NOTE:
// This file was copied/adapted from the Stay Liquid project:
    ...
    ScrollEdgeElementContainer
{
    ...
    func update(
        ...
        if let initialId, let idx = idToIndex[initialId],
            let items = tabBar.items, idx < items.count
        {
    ...
    func select(id: String) {
        guard let idx = idToIndex[id], let items = tabBar.items,
            idx < items.count
        else { return }
        tabBar.selectedItem = items[idx]
            ...
            loadImageIcon(imageIcon) { [weak self] image in
                DispatchQueue.main.async {
                    if let image = image {
                        ...
                        let unselectedImage =
                            self?.createUnselectedImageWithRing(
                                image,
                                imageIcon: imageIcon
                            ) ?? image
                        tabBarItem.image = unselectedImage.withRenderingMode(
                            .alwaysOriginal
                        )

                        // Create selected image with ring (if enabled)
                        let selectedImage =
                            self?.createSelectedImageWithRing(
                                image,
                                imageIcon: imageIcon
                            ) ?? image
                        ...
                        tabBarItem.image =
                            UIImage(systemName: model.systemIcon) ?? UIImage()
                    }
                }
            }
            return
    ...
    private func createSelectedImageWithRing(
        _ image: UIImage,
        imageIcon: ImageIcon
    ) -> UIImage {
        ...
        return ImageUtils.addEnhancedRingToImage(
            image,
            ringWidth: ringWidth,
    ...
    private func createUnselectedImageWithRing(
        _ image: UIImage,
        imageIcon: ImageIcon
    ) -> UIImage {
        guard let ring = imageIcon.ring, ring.enabled else {
    ...
    private class ImageUtils {
        ...
        static func isValidBase64DataUri(_ dataUri: String) -> Bool {
            ...
            return regex?.firstMatch(in: dataUri, options: [], range: range)
                != nil
        }
        ...
        static func loadImageFromBase64(_ dataUri: String) -> UIImage? {
            ...
            let base64String = String(
                dataUri[dataUri.index(after: commaIndex)...]
            )
            ...
            let task = URLSession.shared.dataTask(with: url) {
                ...
                if let contentType = httpResponse.mimeType {
                    ...
                    if !validTypes.contains(contentType.lowercased()) {
                        print(
                            "TabsBar: Unsupported image format: \(contentType)"
                        )
                        DispatchQueue.main.async {
                            completion(nil)
                        }
        ...
        private static func applyImageIconStyling(
            ...
            case "cover":
                resizedImage = resizeImageAspectFill(
                    image,
            ...
            case "fit":
                resizedImage = resizeImageAspectFit(
                    image,
                    targetSize: targetSize
                )
            default:
        ...
        private static func resizeImageToFill(
            _ image: UIImage,
            targetSize: CGSize
        ) -> UIImage {
            UIGraphicsBeginImageContextWithOptions(targetSize, false, 0)
        ...
        static func addEnhancedRingToImage(
            _ image: UIImage,
            ringWidth: CGFloat,
            ringColor: UIColor
        ) -> UIImage {
        ...
        static func addRingToImage(
            _ image: UIImage,
            ringWidth: CGFloat,
            ringColor: UIColor
        ) -> UIImage {
            return addEnhancedRingToImage(
                image,
                ringWidth: ringWidth,
                ringColor: ringColor
            )
